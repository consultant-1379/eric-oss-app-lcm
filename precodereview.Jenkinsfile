...
pipeline {
    ...
    environment {
        ...
        OPEN_API_SPEC_DIRECTORY = "src/main/resources/v3"
    ...
    stages {
        stage('Prepare') {
            steps {
                checkout([$class: 'GitSCM',
                          ...
                          userRemoteConfigs: [
                                  [url: '${GERRIT_MIRROR}/OSS/com.ericsson.oss.ae/eric-oss-app-lcm']
                          ]
                ])
                ...
                parallel(
                        ...
                        "Generate Docs": {
                            ...
                            publishHTML(target: [
                                    ...
                                    keepAll              : true,
                                    reportDir            : 'src/main/resources/v3',
        ...
        stage('Cypress tests') {
            steps {
                script {
                    ansiColor('xterm') {
                        sh "${bob} cypress-tests"
                    }
                }
                archiveArtifacts allowEmptyArchive: true, artifacts: 'eric-oss-app-ui/cypress-allure/* '
            }
        }
        ...
        stage('SonarQube Analysis') {
            ...
            steps {
                withSonarQubeEnv("${env.SQ_SERVER}") {
                    sh "${bob} sonar-enterprise-pcr:generate-version-prefix"
                    sh "${bob} sonar-enterprise-pcr:mvn-sonar-enterprise-sonarqube-analysis"
                }
            }
        }
...
def modifyBuildDescription() {
    ...
    def CHART_NAME = "eric-oss-app-lcm"
    def DOCKER_IMAGE_NAME = "eric-oss-app-lcm"
