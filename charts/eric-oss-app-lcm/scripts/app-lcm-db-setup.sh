...
function command_wrapper {
    ...
    log_message+="\"thread\":\"db consolidation db setup command: $@\","
    log_message+="\"path\":\"/scripts/app-lcm-db-setup.sh\","
...
function init_sql {
    ...
    #===DB-SQL-SCRIPT======
    cat << EOF | psql
CREATE DATABASE $APPLCMPGDATABASE;
CREATE ROLE service_user;
EOF
    #===
   if [ $? -eq 0 ];
     then
       command_wrapper echo "App-mgr db configured for app-lcm db data";
     else
       command_wrapper echo "Error configuring app-mgr db for app-lcm db data";
       exit 1;
   fi
}
...
if psql -lqt | cut -d \| -f 1 | grep -qw $APPLCMPGDATABASE; then
    command_wrapper echo "App-lcm db exists in app-mgr db, no action required";
else
    init_sql
    command_wrapper echo "The app-mgr db is ready for app-lcm db data"
fi
