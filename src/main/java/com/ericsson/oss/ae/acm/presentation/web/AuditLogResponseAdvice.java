...
import static com.ericsson.oss.ae.acm.common.constant.AuditLoggingConstants.PREFERRED_USERNAME_CLAIM_KEY;
import static com.ericsson.oss.ae.acm.common.constant.AuditLoggingConstants.PROPAGATED_HEADER_KEY;
import static com.ericsson.oss.ae.acm.common.constant.AuditLoggingConstants.UPN_CLAIM_KEY;
...
import com.ericsson.oss.ae.acm.presentation.web.configuration.AuditLogProperties;
...
public class AuditLogResponseAdvice implements ResponseBodyAdvice<Object>, Ordered {
    ...
    private void generateAccessLog(Object body, ServerHttpRequest request, ServerHttpResponse response) {
        ...
        if (Arrays.asList(nativeWebRequest.getAttributeNames(NativeWebRequest.SCOPE_REQUEST)).contains(ADDITIONAL_MESSAGE_KEY)) {
            additionaMessage = (String)nativeWebRequest.getAttribute(ADDITIONAL_MESSAGE_KEY, NativeWebRequest.SCOPE_REQUEST);
        }
        ...
        auditLogger.log(Integer.toString(responseLength), Integer.toString(statusCode), preferredUsername, request.getMethod().name(),
            request.getURI(), additionaMessage);
    }
    ...
    private boolean shouldGenerateAccessLog(final ServerHttpRequest request) {
        return auditLogProperties.getIncludePattern().stream().map(this.pathPatternParser::parse)
            ...
            && request.getHeaders().getFirst(PROPAGATED_HEADER_KEY) == null && !request.getMethod().name().equals("GET");
    }
