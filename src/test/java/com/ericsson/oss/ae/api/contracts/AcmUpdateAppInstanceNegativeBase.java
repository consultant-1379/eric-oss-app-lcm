...
public class AcmUpdateAppInstanceNegativeBase extends AcmAppInstancesBase {
    @Override
    public void validate() {
        // 400 Bad Request
        given(service.updateAppInstance(eq(EEFD_22232), any(UpdateAppInstanceRequest.class))).willThrow(
            new AppLcmException(HttpStatus.BAD_REQUEST, BAD_REQUEST_ERROR));

        // 404 Not Found
        given(service.updateAppInstance(eq(EEFD_22230), any(UpdateAppInstanceRequest.class))).willThrow(
            new AppLcmException(HttpStatus.NOT_FOUND, APP_NOT_FOUND_ERROR));

        // 500 Internal Server Error
        given(service.updateAppInstance(eq(EEFD_22231), any(UpdateAppInstanceRequest.class))).willThrow(
