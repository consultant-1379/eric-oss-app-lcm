...
public class AcmGetAppInstanceNegativeBase extends AcmAppInstancesBase {

    @Override
    public void validate() {

        // 500 Internal Server Error
        given(service.getAppInstanceById(eq(EEFD_22232))).willThrow(
            new AppLcmException(HttpStatus.INTERNAL_SERVER_ERROR, DEFAULT_APP_LCM_ERROR));

        // 404 Not Found
        given(service.getAppInstanceById(eq(EEFD_22231))).willThrow(
            new AppLcmException(HttpStatus.NOT_FOUND, APP_NOT_FOUND_ERROR));

        // 400 Bad Request
        given(service.getAppInstanceById(eq(EEFD_22230))).willThrow(
