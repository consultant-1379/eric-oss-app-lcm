...
public class AcmEnableDisableAppNegativeBase extends AcmAppsBase {
    @Override
    public void validate() {
        // 500 Internal Server Error
        given(service.enableDisableApp(EEFD_22231, new EnableDisableAppRequest().mode(AppMode.ENABLED))).willThrow(
            new AppLcmException(HttpStatus.INTERNAL_SERVER_ERROR, DEFAULT_APP_LCM_ERROR));

        // 404 Not Found
        given(service.enableDisableApp(EEFD_22232, new EnableDisableAppRequest().mode(AppMode.ENABLED))).willThrow(
            new AppLcmException(HttpStatus.NOT_FOUND, APP_NOT_FOUND_ERROR));

        // 400 Bad Request
        given(service.enableDisableApp(EEFD_22233, new EnableDisableAppRequest().mode(AppMode.ENABLED))).willThrow(
