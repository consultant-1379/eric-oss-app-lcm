...
checkServiceYAML(){
    SERVICE_NAME=$1
    echo -e "SKIPPED: due to $SERVICE_NAME applies scrape role [service] according to the DR-D470223-010.\nPlease refer to the page provided:\n$METRICS_DESINGE_RULES";
#    if grep -q "{{- include \"$SERVICE_NAME.annotationsWithPrometheus\" . | nindent [0-9] }}" ./charts/$SERVICE_NAME/templates/service.yaml; then
#        echo "SUCCESS: service.yaml containes all the lines necessary for metrcis exposure.";
#        echo "true" >> .bob/var.metrics-exposed;
#    else
...
#        echo -e "What is needed:"
#        echo -e "{{- include \"$SERVICE_NAME.annotationsWithPrometheus\" . | nindent [0-9] }}"
...
checkDeploymentYAML(){
    SERVICE_NAME=$1
    if grep -q "{{- include \"$SERVICE_NAME.annotationsWithPrometheus\" . | nindent [0-9] }}" ./charts/$SERVICE_NAME/templates/deployment.yaml; then
        echo "SUCCESS: deployment.yaml containes all the lines necessary for metrcis exposure.";
        echo "true" >> .bob/var.metrics-exposed;
    else
...
checkHelperTPL(){
    ...
    if cmp -s .bob/var.compareToLine .bob/var.scrapedLine; then
        echo "SUCCESS: helper.yaml containes all the lines necessary for metrcis exposure.";
        echo "true" >> .bob/var.metrics-exposed;
    else
