...
export function validateFormField(dialog, fieldName, value) {
  const field = inShadow(dialog, `#${fieldName}`);
  expect(field, 'Field was successfuly created').to.not.equal(null);
  expect(field.value, `Field value match with ${value}`).to.be.equal(value);
}
