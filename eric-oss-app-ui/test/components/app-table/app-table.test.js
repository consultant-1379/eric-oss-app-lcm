...
import { injectHTMLElement} from '../../utils/utils';
import * as sinon from "sinon";
import { restCalls } from "../../../src/config/restCallsConfig";
import { restCallsV1 } from '../../../src/config/restCallsConfigV1.js';

describe('App Table Component Tests', () => {
  let container;
  let inject;
  before(() => {
  ...
  describe('Basic component setup', () => {
    it('should create a new <e-app-table>', async () => {
      const customElement = await inject('<e-app-table></e-app-table>');
      const actualElementRendered = container.querySelector('e-app-table');
      expect(actualElementRendered, '<e-app-table></e-app-table> was not found').to.equal(customElement);
    });

    it('should have 6 columns', async () => {
      const tableElement = await fixture(html` <e-app-table></e-app-table> `);
      const euiTable = tableElement.shadowRoot.querySelector('eui-table');

      expect(euiTable.columns.length).to.eql(6);
    });
  });

  describe('Table Rendering', () => {
    it('should convert to title case', async () => {
      const tableElement = await inject('<e-app-table></e-app-table>');

      expect(tableElement._toTitleCase('TEST')).to.eql('Test');
    });
    ...
    it('should update the table with data', async () => {
      ...
      const appUnderTest = new AppTable;
      ...
      expect(appUnderTest.data, 'The correct detail is returned').to.eql(data);
    });
  });
})